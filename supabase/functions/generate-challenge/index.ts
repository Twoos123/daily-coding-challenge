  ...
  const response = await fetch('https://api.perplexity.ai/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a programming challenge generator specializing in data structures. Generate a unique LeetCode-style coding challenge that focuses on implementing or manipulating common data structures (like arrays, linked lists, trees, graphs, stacks, queues, heaps, or hash tables). Include: 1) Problem Description emphasizing the data structure aspect 2) Example Input/Output 3) Constraints 4) A complete solution in Python with detailed explanation of how the solution uses the data structure effectively. Rate the difficulty from 1-5, but only include this rating in a machine-readable format at the start of your response like "DIFFICULTY:3". Format the rest of the response in markdown with proper headings and code blocks, excluding any mention of difficulty.'
        },
        {
          role: 'user',
          content: 'Generate a new coding challenge focused on data structures'
        }
      ],
      temperature: 0.7,
      max_tokens: 1000,
